...
from borb.pdf import (
    ...
    Lipsum,
    PDF,
    PageLayout, TableOfContents, Heading,
)
...
class TestTableOfContents(unittest.TestCase):
    ...
    def test_table_of_contents_002(self):
        ...
        for i in range(0, 5):
            ...
            for j in range(0, 3):
                l.append_layout_element(Heading(f'Subheading {i+1}.{j+1}', outline_level=1))
                for _ in range(0, 3):
                    l.append_layout_element(Paragraph(Lipsum.generate_lorem_ipsum(300), font_size=12))
